...
- **AI custom workouts** — describe a session in plain English and get a structured workout
  (a Netlify serverless function calls the Claude API).
...
one Netlify serverless function for the AI generator · installable PWA (manifest + service worker).

## Run locally
Open `index.html` in a browser (double-click). The AI generator only runs on the deployed site,
since it needs the serverless function + `ANTHROPIC_API_KEY`.

## Deploy (Netlify)
Push to this repo and connect it to Netlify for auto-deploy, or drag the folder onto the site's
Deploys tab. The AI function needs an `ANTHROPIC_API_KEY` environment variable set in the Netlify
site settings. Database schema lives in `phase2/*.sql` (run in the Supabase SQL editor).
...
Anthropic API key lives only as a Netlify environment variable.
